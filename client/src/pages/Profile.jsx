    ...
    >
      ...
      <section className=" px-3 sm:px-12 md:px-0 xl:px-20">
        <div className="flex items-center gap-4 sm:gap-12">
          <img className={`size-20 sm:size-28 md:size-32 ${isDark?'invert':''}`} src={profilePic} alt="" />
          <div>
...
<section className="px-3 sm:px-12 md:px-0 xl:px-20">
  ...
  <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-3 xl:grid-cols-4 gap-3 mb-3">

    {assets.map((pic,index)=>{
return (
  <div key={index} className="size-24 sm:size-32 md:size-36 lg:size-40 my-1 transition-all hover:scale-105 post rounded-md overflow-hidden max-w-38 flex justify-center items-center">
      <img className="w-full h-full rounded-md object-cover object-center" src={pic} alt="" />
    </div>
)
    })}
  </div>
